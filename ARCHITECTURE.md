...
**UI chrome rule:** Toolbar, rulers and window controls are UI chrome. They must not be clipped, transformed or hidden by sheet content. Concretely: chrome lives outside/above the sheet z-range (ruler `z-index: 30`, toolbar `z-index: 50`, sheets and lifted active-sheet groups stay ≤ 10 + drawing surface 20), is never a child of a sheet group, and adapts to the window instead of overflowing it — the toolbar caps its width to the viewport and wraps into extra rows (growing upward from its bottom anchor) when the window is too narrow for one row, so every control stays reachable at any window size.
